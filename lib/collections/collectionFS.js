...
export const Media = new FS.Collection("Media", {
  stores: [
    ...
    // XXX: GETOUTFITTED MOD: Add GetOutfitted specific sizing for uploaded images.
    new FS.Store.GridFS("cart", {
      ...
      transformWrite: function (fileObj, readStream, writeStream) {
        if (gm.isAvailable) {
          gm(readStream, fileObj.name).resize("1600", "1200").stream()
            .pipe(writeStream);
        } else {
          readStream.pipe(writeStream);
        }
      }
    }), new FS.Store.GridFS("portrait", {
      chunkSize: 1 * 1024 * 1024,
      transformWrite: function (fileObj, readStream, writeStream) {
        if (gm.isAvailable) {
          gm(readStream, fileObj.name).resize("1200", "1600").stream()
            .pipe(writeStream);
        } else {
          readStream.pipe(writeStream);
        }
