...
export const FedExApi = function (address) {
  check(address, {
    address1: String,
  ...
  if (!auth) {
    Logger.warn('FedEx Api credentials are not congfigured');
    return false;
  }
  ...
  if (!rates.RateReplyDetails) {
    Logger.warn('FedEx did not return rates');
    return false;
  }
  let groundRate = rates.RateReplyDetails[0];
  Logger.info('FedEx returned TransitTime');
